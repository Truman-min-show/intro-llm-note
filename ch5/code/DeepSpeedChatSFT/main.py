...
def main():
    ...
    # 使用 HuggingFace 的AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, trust_remote_code=True)
    # 有些模型（比如Qwen）已经有pad_token了，如果没有，可以手动加
    ...
    # Prepare the data
    train_phase = 1
    from dschat.utils.data.raw_datasets import MyDataset
    ...
    def evaluation(model, eval_dataloader):
        ...
        for step_idx, batch in enumerate(eval_dataloader):
            try:
                batch = _prepare_batch_for_model(batch, device)  # device closed over from outer scope in your main
            except Exception as e:
                # Helpful debug message and re-raise
                print_rank_0(f"Error preparing batch for model: {e}", args.global_rank)
                print_rank_0(f"Offending raw batch type: {type(batch)}; sample keys/info: {getattr(batch, 'keys', lambda: None)()}", args.global_rank)
                raise
