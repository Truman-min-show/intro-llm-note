...
def my_load(filepath, tokenizer, max_seq_len=512, seed=42):
    import json
    ...
    def tokenize_fn(example):
        # 这里拼接 prompt + chosen
        # 你可以根据需要调整模板，比如是否加换行符
        text = "Human: " + example["prompt"] + "\nAssistant: " + example["chosen"]
        tokenized = tokenizer(
            text,
            truncation=True,
            max_length=max_seq_len,
            padding="max_length"  # 如果你想动态 padding，可以改成 padding=False
        )
        input_ids = tokenized["input_ids"]
        # 将 pad token 位置设为 -100（labels 忽略位）
        pad_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
        labels = [ (t if t != pad_id else -100) for t in input_ids ]
...
class MyDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name, tokenizer=None, max_seq_len=512):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = dataset_name
        self.dataset_name_clean = "MyDataset/CVPR2024Summary"

        # 传入 tokenizer 来做 tokenization
        assert tokenizer is not None, "You must pass a tokenizer to MyDataset"
        self.raw_datasets = my_load(dataset_name, tokenizer, max_seq_len=max_seq_len, seed=seed)
